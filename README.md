...
4. Season Pass (**not working as of now, however a fix is really close**), contains all the benefits of the season pass (double fashion points, event lives etc.) however claiming the tiers will return an error (impossible) (**Unbannable**) (**Safe**)
...
As of now, Hyperkani domains trust the **mitmproxy certificate** and the app is not SSL pinned, so a drawback has not been raised for that point with Hyperkani domains.
